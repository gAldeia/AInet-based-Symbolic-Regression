...
linearExpression n = [(Coeff 1.0, 0, Exps e) | e <- (exps n)]
    where
...
randomIt n = do
    ...
    exps <- randomExps n
    return (Coeff 1.0, randOp, Exps exps)
...
textRepresentation le = init $ concat [monomio it | it <- le]
    where
        monomio (c, o, Exps e) = show c ++ "*" ++ nome o ++ exps e
...
solve le (xs, y) = [(eval o e) * c | ( Coeff c, o, Exps e) <- le]
    where
...
adjust le ds = [(Coeff c, o, e) |  (c, (_, o, e)) <- zip coeffs le]
    where
...
simplify le sT
    ...
    where
        simplified = [(Coeff c, o, e) | (Coeff c,o,e) <- le, c >= sT]
...
mutateInter n le = do
    ...
    let newIt = [(c, o, Exps randExps) | (c, o, e) <- [le !! chosenIndex]]
    let le' = [it | it<-le, it /= (le !! chosenIndex)]
